using MarketAlly.Maui.ViewEngine.Events;
...
namespace MarketAlly.Maui.ViewEngine.Platforms.Windows
{
    internal class PdfViewHandler : ViewHandler<IPdfView, ScrollViewer>
    {
        internal static readonly PropertyMapper<PdfView, PdfViewHandler> PropertyMapper = new(ViewMapper)
        {
        ...
        async Task RenderPages()
        {
            ...
            // Get display density on UI thread before background work
            double density = 1.0;
            var densityTcs = new TaskCompletionSource<double>();
            _scrollViewer.DispatcherQueue.TryEnqueue(() =>
            {
                try
                {
                    densityTcs.SetResult(DeviceDisplay.MainDisplayInfo.Density);
                }
                catch
                {
                    densityTcs.SetResult(1.0); // Fallback to default density
                }
            });
            density = await densityTcs.Task;
            ...
            for (uint i = 0; i < pdfDoc.PageCount; i++)
            {
                ...
                using (InMemoryRandomAccessStream stream = new())
                {
                    ...
                    // Create bitmap and page element on UI thread
                    var addTcs = new TaskCompletionSource<bool>();
                    _scrollViewer.DispatcherQueue.TryEnqueue(async () =>
                    {
                        try
                        {
                            ...
                            var pageElement = MakePage(bitmap, _pageAppearance);
